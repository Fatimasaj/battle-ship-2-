    ...
    while(True):
        ...
        if len(alphaX) == 1:
            if not alphaX.isnumeric():
                x = ord(alphaX)-97
            elif alphaX.isnumeric(): 
                x = int(alphaX)
            if x <= 9 or x >= 0:
                return x
            else:
                print("Make sure you input a letter between A and J.")
        else:
...
if p == "y" or p == "yes":
    for i in range(len(SHIPLENGTH)):
        while (True): 
            ...
            y = verticalPosition()
            o = orientation()
            placement = placeShip(x, y, o, SHIPLENGTH[i-1], mine, SHIPSYMBOL[i-1])
            print()
        if placement == True:
            break 
